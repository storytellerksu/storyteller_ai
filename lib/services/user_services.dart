...
class UserServices {
  ...
  Future emailRegistration(String email, String password, username) async {
    try {
      UserCredential result = await authKey.createUserWithEmailAndPassword(
          email: email, password: password);
      User user = result.user!;

      await DatabaseServices(uid: user.uid).updateUserName(username);
      return userIDFromUser(user);
  ...
  Future getUsername() async {
    return DatabaseServices(uid: authKey.currentUser!.uid).fetchUsername();
  }
}
